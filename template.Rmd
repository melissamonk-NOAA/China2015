...
#########################################################################
  ...
  #spawning biomass and std.dev data, calculate lower and upper 95% CI                 
      ...
      colnames(SpawningBtab) = c('Year','Spawning Output (MM eggs)','~ 95% confidence interval')
...
This stock assessment estimates that China rockfish in the north are above the biomass target. The spawning output of the stock declined between the 1960s and 1990s but has largely been stable during the past few decades.  The estimated relative depletion level in 2015 is 51.5% (~95% asymptotic interval: $\pm$ 47.2% - 55.8%, corresponding to 33.4 mt (~95% asymptotic interval: 27.4 – 39.4 mt) of spawning output in the base model (Table \ref{tab:Ref_pts_N}).  Unfished spawning stock biomass was estimated to be 64.8 mt in the base case model. The target stock size based on the biomass target ($SB_{40\%}$) is 25.9 mt, which gives a catch of 3.3 mt. Equilibrium yield at the proxy $F_{MSY}$ harvest rate corresponding to $SPR_{50\%}$ is 3.1 mt.

This stock assessment estimates that central area China rockfish are just above the biomass target. The rate of spawning output decline is estimated to be steepest during the 1980s to 1990s and has continued to decline since the 1990s at a slower rate. The estimated relative depletion level in 2015 is 42.0% (~95% asymptotic interval: $\pm$ 38.4% - 45.6%), corresponding to 85.9 mt (~95% asymptotic interval: 73.1 – 98.7 mt) of spawning output in the base model (Table \ref{tab:Ref_pts_C}). Unfished spawning stock biomass was estimated to be 204.5 mt in the base case model. The target stock size  based on the biomass target ($SB_{40\%}$) is 81.8 mt, which gives a catch of 9.4 mt. Equilibrium yield at the proxy $F_{MSY}$ harvest rate corresponding to $SPR_{50\%}$ is 8.8 mt. 

This stock assessment estimates that China rockfish south of $40^\circ 10^\prime$ N. latitude are below the biomass target, but above the minimum stock size threshold, and have been increasing over the last 15 years. The estimated relative depletion level in 2015 is 29.6% (~95% asymptotic interval: $\pm$ 25.0% - 34.3%), corresponding to 111.5 mt (~95% asymptotic interval:  89.9 – 133.1 mt) of spawning output in the base model (Table \ref{tab:Ref_pts_S}). Unfished spawning stock biomass was estimated to be 376.5 mt in the base case model. The target stock size  based on the biomass target ($SB_{40\%}$) is 150.6 mt, which gives a catch of 20.8 mt. Equilibrium yield at the proxy $F_{MSY}$ harvest rate corresponding to $SPR_{50\%}$ is 19.3 mt. 
...
Quantity= c('Unfished spawning output (MM eggs)',
            'Unfished age 0+ biomass (mt)',
            'Unfished recruitment (R0, thousands)',
            'Spawning output (2015)',
            'Depletion (2015)',
...
All models used the fecundity relationship from Dick [-@Dick2009].
...
Retrospective analyses were conducted for each model by conducting model runs that sequentially remove the last year of data over the last 5 base model years. The southern model showed very little change in estimated spawning output trajectory as a result of this data removal (Figure \ref{fig:S_retroS_compare1_spawnbio}). The central and northern models, however, showed that the each additional year of data added to the model has resulted in a higher initial spawning output (Figures \ref{fig:C_retroC_compare1_spawnbio} and \ref{fig:N_retroN_compare1_spawnbio}). These results are consistent with the dependence of the central and northern models on more recently collected data as compared to the southern model where the catch history began earlier.
...
<!-- ******************************************************************************************************************************************** --> 
![Comparison of depletion among the 2013 data moderate assessment, a SS3 bridge model, and the 2015 base case for the combined nothern and central models.   \label{fig:STARrequest2_CN}](plots/STARrequest2_CN.png)
