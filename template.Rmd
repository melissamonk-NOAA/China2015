...
The previous assessment of China rockfish [@Cope2015_Data-Moderate_Assessment] modeled two China rockfish populations, north and south of **fix this: 40-10** North latitude (roughly Cape Mendocino). The majority of landings occurred south of Cape Mendocino, and the revised estimates are substantially lower in early years, primarily due to the removal of trawl catch (Figure \ref{EJD_CALCOM_3}).

California’s commercial live-fish fishery began targeting nearshore rockfish species in the early 1990s [@CDFG_CalCOFI_2002]. Live annual landings of China rockfish surpassed landings of dead fish by the late 1990s, due to the increased value of fish landed live (Figure \ref{EJD_CALCOM_4}).
...
We generated separate indices in California for the 1987-1999 and 2000-2014 datasets due to the number of regulation changes occurring throughout the time period (see Appendix H, pp.\pageref{sec:AppendixH}).  CDFW implemented a regulation of three hooks in 2000, which was reduced to (and remains at) two hooks in 2001.
...
The selected data contained categorical variables for YEAR (13 levels), WAVE (2 levels), REGION (2 levels, north and south of Florence), and three depth bins (DEPTH: 0-19 m, and 20-59 m). A lognormal model was selected over a gamma for the positive encounters by a deltaAIC of 20.01. Model selection, using AIC, selected a lognormal model with YEAR, WAVE, DEPTH, REGION, and a WAVE:DEPTH interaction, while a binomial with YEAR, REGION, and WAVE was selected.  In the lognormal submodel, stepwise BIC retained the YEAR. In the binomial model, stepwise BIC retained REGION and WAVE. The final YEAR effects from the delta-GLM with main effects YEAR, WAVE and REGION are shown in Table \ref{tab:MHM_OR_onboard} and Figure \ref{fig:MHM_OR_onboard}).
...
The filtered dataset included 13,993 drifts, of which 1,403 (10\%) drifts with positive encounters.
...
```{r, eval=TRUE, echo=FALSE, message=FALSE, warning=FALSE, include=FALSE}
OR_onboard = read.csv("C:/China2015/txt_files/OR_onboard.csv")

OR_onboard1 = OR_onboard[-2,c(1,2,3)]
colnames(OR_onboard1) = c('Year','Index','Log-scale SE')
```
```{r, eval=TRUE, echo=FALSE, results='asis', message=FALSE}
design.table = xtable(OR_onboard1,caption=c("Year effects for the Oregon onboard observer index"),label='tab:OR_onboard',digits=4)

print(design.table, include.rownames=FALSE, caption.placement="top",tabular.environment="tabularx",width="\\textwidth")
```
...
<!-- Recreational onboard observer --> 
```{r,eval=TRUE, echo=FALSE, message=FALSE, warning=FALSE,fig.cap="Index for the Oregon onboard observer program, with 95% lognormal confidence intervals."}
...
![Map of the reefs near Pt. Reyes, CA (a) and overlaid with the fishing location codes from the CDFW 1987-1998 onboard observer program.\label{MHM_careefs}](plots//MHM_CA_reefmap.png)
...
The following commercial regulations pertain to China rockfish species in washington and were provided by the California Department of Fish and Wildlife.  There has been a 12 inch minimum size limit on China rockfish since 2001.
...
\begin{description}
    \item[2003] \hfill\\
        Split into shallow and deep nearshore restricted access permits, with shallow nearshore permits in 4 regions and the deep nearshore permits statewide
        
        Trip limits for restricted access fishery, with differential trip limits north and south of $40^{\circ} 10' N$ for both permits, and only the deep   nearshore permits south of Point Conception
        
        Subject to depth restrictions consitsten wit the shoreward non-trawl RCA
