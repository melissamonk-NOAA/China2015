...
This assessment reprots the states of the China rockfish \emph{Sebastes nebulosus} resource off the coast of the California (north of Pt. Conception $34^{\circ} 27'$ N latitude), Oregon, and Washington using data through 2014.  China rockfish are modeled as........
...
California’s commercial live-fish fishery began targeting nearshore rockfish species in the early 1990s [@CalCOFI2002]. Live annual landings of China rockfish surpassed landings of dead fish by the late 1990s, due to the increased value of fish landed live (Figure \ref{fig:EJD_CALCOM_4}).
...
Model selection procedures identified the covariates \emph{vessel, port, depth and people} as important, and along with the categorical \emph{year} factor of interest for the index were the variables included in both the catch occurrence and positive catch component models.  Extracted, back-transformed and bias corrected estimates of the \emph{year} effect were used for the abundance index (Table \ref{tab:OR_commlog}, Figure \ref{fig:OR_commlog_pos}). A jackknife resampling routine was conducted to estimate the standard error (and CV) of the \emph{year} effects.  A jackknife resampling routine was conducted to estimate the standard error (and CV) of the year effects.  The relative effects of each covariate are shown in Figure \ref{fig:OR_commlog_covall} for the catch occurrence component and Figure \ref{fig:OR_commlog_covpos} for the positive catch component.  Standard model diagnostics show adequate fit and general consistency with GLM model assumptions for the positive catch component (Figure \ref{fig:OR_commlog_covpos}).
...
Model selection procedures selected the covariates \emph{Month} and \emph{Boat type} as important for both the catch occurrence and positive catch component models, and along with the categorical year factor used for the index of abundance (Tables \ref{tab:WA_dock_aic1t} and \ref{tab:WA_dock_aic2t}).  A bootstrap analysis (N=500) was used to estimate the standard errors (and CVs) of the year effects (Table \ref{tab:WAdock}). Standard model diagnostics show adequate fit and general consistency with GLM model assumptions for the positive catch component Figures \ref{fig:WA_dock_diagSM} adn \ref{fig:WA_dock_gamma}).
...
#WA dockside table of summary sample sizes
 WA_dock.samplet = xtable(WA_dock_sample,caption=c("WDFW recreational dockside data sample sizes at each data filtering step.  The bold value indicates the final sample size used for delta-GLM analysis."),label='tab:WAdock_aic',digits=0)
align(WA_dock.samplet)='lllcc'
 print(WA_dock.samplet, include.rownames=FALSE, caption.placement="top",tabular.environment="tabularx",width="\\textwidth",hline.after=c(-1,2,nrow(WA_dock.samplet)),align=TRUE)
```


```{r, eval=TRUE, echo=FALSE, results='asis', message=FALSE}
#WA dockside table aic1
 WA_dock.aic1t = xtable(WA_dock_aic1,caption=c("AIC values for each model using the data with Stephens-MacCall filtering."),label='tab:WAdock_aic1',digits=0)
...
#WA dockside table aic2
 WA_dock.aic2t = xtable(WA_dock_aic2,caption=c("AIC values for each model using the data without Stephens-MacCall filtering."),label='tab:WAdock_aic2',digits=0)
...
#WA dockside table index
 WA_dock.table = xtable(WA_dock_all,caption=c("Washington recreational China rockfish CPUE indices."),label='tab:WAdock',digits=4)
align(WA_dock.table)='llcccccc'
 print(WA_dock.table, include.rownames=FALSE, caption.placement="top",tabular.environment="tabularx",width="\\textwidth",hline.after=c(-1,0,1,nrow(WA_dock.table)),align=TRUE)
```
...
<!-- ******************************************************************************************************************************************** --> 
<!-- Recreational onboard observer TABLES--> 
```{r, eval=TRUE, echo=FALSE, message=FALSE, warning=FALSE, include=FALSE}
...
<!-- Recreational Landings FIGURES--> 

<!-- Commercial Discards FIGURES--> 

<!-- Recreational Discards FIGURES--> 
...
![Characterization of the final subset of logbook data used in delta-GLM analyses for China rockfish. \label{fig:or_commlog_sample}](plots/OR_commlog_sample.png)

\newpage

![The distribution of set-level raw positive catch CPUE data relative to potential covariates evaluated in the China rockfish delta-GLM analysis. \label{fig:or_commlog_pos}](plots/OR_commlog_pos.png)
...
![Summary of the relative effects of each covariate in the positive catch model component for China rockfish. \label{fig:or_commlog_covpos}](plots/OR_commlog_covpos.png)

\newpage

![Diagnostic plots for the China rockfish positive catch component delta-GLM model.  These are used to evaluate model fit (top left), assumptions of normality (top right), assumptions of constant variance (bottom left), and the presence of outliers (bottom right).  \label{fig:or_commlog_diag}](plots/OR_commlog_diag.png)
...
![Summary data plots for the data set with Stephens-MacCall filtering. \label{WA_dock_summarySM}](plots/WA_dock_summarySM.png)

![Diagnostic plots for the China rockfish positive catch component longnormal delta-GLM model for the dataset with Stephens-MacCall filtering.  These are used to evaluate model fit (top left), assumptions of normality (top right), assumptions of constant variance (bottom left), and the presence of outliers (bottom right). \label{WA_dock_diagSM}](plots/WA_dock_diagSM.png)

![Diagnostic plots for the China rockfish positive catch component gamma delta-GLM model for the dataset without Stephens-MacCall filtering.  These are used to evaluate model fit (top left), assumptions of normality (top right), assumptions of constant variance (bottom left), and the presence of outliers (bottom right).  \label{WA_dock_gamma}](plots/WA_dock_gamma.png)
