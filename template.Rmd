...
#read in the SS output file
 myreplist = SS_output(dir='C:/Aurora_rockfish_assessment', covar=TRUE, ncol=1000)
...
```{r, eval=TRUE, echo=FALSE, results='asis', message=FALSE}
  ...
  align(SpawningB.table) = c('l','l','>{\\centering}p{1in}','>{\\centering}p{1.2in}')  #xtable requires a placeholder for rownames, hence 4 values
...
![Time series of spawning biomass trajectory (circles and line: median; light broken lines: 95% credibility intervals) for                          China rockfish. \label{spawningB}](spawningB)

This is a test to see if I can reference Figure \ref{spawningB}. 


![sdfafafasfaf](C:/Aurora_rockfish_assessment/plots/recdevs1_points.png)
![testtest](C:/Aurora_rockfish_assessment/plots/ts7Spawningbiomass(mt).png)
...
##Appendix A.1 Sub-headings in Appendix
