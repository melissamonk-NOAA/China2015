...
Much research is needed to elucidate the role of China rockfish in the ecosystem.  


##Fishery Information

China rockfish are most commonly captured by hook and line or traps.  They are rarely encountered in the trawl fishery due to the elusive behavior and affinity for rocky crevices.  Their high site fidelity and territoriality lend to the difficulty of capturing the species.  
...
Trips that had erroneous trips hours as entered on paper and then entered electronically later. These were the steps taken to correct the issue:
...
Table \ref{tab:MRFSS_S_filter}
Table \ref{tab:MRFSS_S_sample_size}
Table \ref{tab:MRFSS_S_aic}
Table \ref{tab:MRFSS_S_index}

Figure \ref{fig:MRFSS_S_SM}
Figure \ref{fig:MRFSS_S_diag}

**California MRFSS Dockside Charter Boat Index, North of $40^\circ 10^\prime$**
\hl{EJ}

Table \ref{tab:MRFSS_N_filter}
Table \ref{tab:MRFSS_N_sample_size}
Table \ref{tab:MRFSS_N_aic}
Table \ref{tab:MRFSS_N_index}

Figure \ref{fig:MRFSS_N_SM}
Figure \ref{fig:MRFSS_N_diag}
...
```{r, eval=TRUE, echo=FALSE, results='asis', message=FALSE}
        #MRFSS N aic
#         MRFSS_N_aic.table = xtable(MRFSS_N_aic,caption=c("AIC values for each model in the Oregon and Northern California MRFSS dockside index."),
#                                            label='tab:MRFSS_N_aic')
#        align(MRFSS_N_aic.table) = c('l','>{\\raggedright}p{2.5in}','>{\\centering}p{1in}','>{\\centering}p{1in}')          
#        print(MRFSS_N_aic.table, include.rownames=FALSE, caption.placement="top",tabular.environment="tabularx",width="\\textwidth",
#                         sanitize.text.function = function(x){x})
```
\FloatBarrier
```{r, eval=TRUE, echo=FALSE, results='asis', message=FALSE}
...
#        print(MRFSS_N_index1.table, include.rownames=FALSE, caption.placement="top",tabular.environment="tabularx",width="\\textwidth",
#                         sanitize.text.function = function(x){x})
```
...
<!-- MRFSS S--> 
![Species coefficients from the binomial GLM for presence/absence of China rockfish in the California MRFSS South data set. Horizontal bars are 95\% confidence intervals. Albacore coefficient (<-10) excluded for scale.  \label{fig:MRFSS_S_SM}](plots/MRFSS_S_SM.png)


![Diagnostic plots for the China rockfish delta-GLM index (lognormal component) for the California MRFSS South data set.  These are used to evaluate model fit (top left), assumptions of normality (top right), assumptions of constant variance (bottom left), and the presence of outliers (bottom right). \label{fig:MRFSS_S_diag}](plots/MRFSS_S_diag.png)

<!-- MRFSS N--> 

![Species coefficients from the binomial GLM for presence/absence of China rockfish in the California MRFSS North data set. Horizontal bars are 95\% confidence intervals. Albacore coefficient (<-10) excluded for scale.  \label{fig:MRFSS_N_SM}](plots/MRFSS_N_SM.png)


![Diagnostic plots for the China rockfish delta-GLM index (lognormal component) for the California MRFSS North data set.  These are used to evaluate model fit (top left), assumptions of normality (top right), assumptions of constant variance (bottom left), and the presence of outliers (bottom right). \label{fig:MRFSS_N_diag}](plots/MRFSS_N_diag.png)
